...
export function ElevatedCardWidget({ name, userid, phone, headImg, status, onClick }: ElevatedCardWidgetProps) {
  ...
  return (
    <div
      ...
      className="cursor-pointer transition-all duration-200 ease-out rounded-xl"
      style={{
        transform: isHovered ? 'translateY(-4px)' : 'translateY(0)',
        boxShadow: isHovered ? '0 8px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)' : 'none'
      }}
    >
      <Card className="transition-shadow duration-200" style={{ backgroundColor: 'var(--surface-secondary)' }}>
