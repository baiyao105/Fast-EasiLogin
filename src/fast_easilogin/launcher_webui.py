...
def add_hosts_entry() -> bool:
    if check_hosts_entry():
        print(f"[OK] hosts 映射已存在: {HOSTS_IP}    {HOSTS_ENTRY}")
        return True
    ...
    try:
        ...
        print("[X] 权限不足, 无法修改 hosts 文件")
        print("    请以管理员身份运行此脚本")
        return False
    except Exception as e:
        print(f"[X] 添加 hosts 映射失败: {e}")
        return False
    else:
        print(f"[OK] hosts 映射已添加: {HOSTS_IP}    {HOSTS_ENTRY}")
        return True
...
def install_service() -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            print("[OK] Windows 服务安装完成")
            print(f"    可通过 'sc query {SERVICE_NAME}' 查看服务状态")
            return True
        print(f"[X] 安装服务失败: {result.stderr or result.stdout}")
        return False
...
def uninstall_service() -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            print("[OK] Windows 服务卸载完成")
            return True
        print(f"[X] 卸载服务失败: {result.stderr or result.stdout}")
        return False
...
def main() -> None:
    ...
    if args.install and args.uninstall:
        print("[X] 错误: 不能同时指定 --install 和 --uninstall")
        sys.exit(1)
